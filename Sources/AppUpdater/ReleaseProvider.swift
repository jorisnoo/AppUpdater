...
public protocol ReleaseProvider {
    func fetchReleases(owner: String, repo: String, urlTransform: URLTransform?) async throws -> [Release]
    func download(asset: Release.Asset, to saveLocation: URL, urlTransform: URLTransform?) async throws -> AsyncThrowingStream<DownloadingState, Error>
}
...
public struct GithubReleaseProvider: ReleaseProvider, Sendable {
    ...
    public func fetchReleases(owner: String, repo: String, urlTransform: URLTransform?) async throws -> [Release] {
        ...
        guard let task = try await URLSession.shared.dataTask(with: url, urlTransform: urlTransform)?.validate() else {
    ...
    public func download(asset: Release.Asset, to saveLocation: URL, urlTransform: URLTransform?) async throws -> AsyncThrowingStream<DownloadingState, Error> {
        return try await URLSession.shared.downloadTask(with: asset.downloadUrl, to: saveLocation, urlTransform: urlTransform)
    }
}
